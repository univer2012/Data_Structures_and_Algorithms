...
//一次归并
/* 设两个相邻的有序序列为r[s] ~ r[m] 和 r[m+1] ~ r[t]，将这两个有序序列归并成一个有序序列r1[s] ~ r1[t]。
 * 为此，设三个参数i、j和k分别指向两个待归并的有序序列和最终有序序列的当前记录，
 * 初始时i、j分别指向两个有序序列的第一个记录，即i = s，j = m + 1，k指向存放归并结果的位置，即k = s。
 * 然后比较i和j所指记录的关键码，取出较小者作为归并结果存入k所指位置，
 * 直至两个x有序序列之一的所有记录都取完，再将另一个有序序列的剩余记录顺序送到归并后的有序序列中。
 */
